...
def train_advent(model, trainloader, targetloader, cfg):
    ...
    for i_iter in tqdm(range(cfg.TRAIN.EARLY_STOP + 1)):
        ...
        #n_aux,c_aux,w_aux,h_aux = pred_src_aux.shape
        #interp_tmp = nn.Upsample(size=(w_aux, h_aux), mode='bilinear',
         #                    align_corners=True)
        #pred_src_main_fuse = interp_tmp(pred_src_main) + pred_src_aux * 0.5 
        ...
        #pred_trg_main_fuse = interp_tmp(pred_trg_main) + pred_trg_aux * 0.5
        if cfg.TRAIN.MULTI_LEVEL:
