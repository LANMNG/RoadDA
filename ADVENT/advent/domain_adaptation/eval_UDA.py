        ...
        if i_iter not in all_res.keys():
            ...
            for index in tqdm(range(len(test_loader))):
                ...
                with torch.no_grad():
                    ...
                    #pred_aux, pred_main = models[0](image.cuda(device))
                    #n_aux,c_aux,w_aux,h_aux = pred_aux.shape
                    #interp_tmp = nn.Upsample(size=(w_aux, h_aux), mode='bilinear',align_corners=True)
                    #pred_main_fuse = interp_tmp(pred_main) + pred_aux * 0.5
